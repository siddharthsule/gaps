...
__device__ double hadronisation::f_reshuffling(double k, double* masses,
                                               double ecms, event& ev) const {
  /**
...
__global__ void constituent_reshuffling(event* events, hadronisation* had,
  ...
  for (int i = 2; i < ev.get_size(); i++) {
    ...
    if (ev.get_particle(i).get_pid() == 21) {
      masses[i - 2] = had->gluon_mass;  // gluon
    } else {
      int fl = abs(ev.get_particle(i).get_pid());  // quark
      masses[i - 2] = had->const_mass[fl - 1];
    }
  }
  ...
  for (int iter = 0; iter < 50; iter++) {
    ...
    if (had->f_reshuffling(kmid, masses, ecms, ev) > 0.0) {
      khi = kmid;
    } else {
      klo = kmid;
    }
