...
__global__ void cluster_genkt(const event* events, double* results, int n) {
  ...
  results[20 * idx + 8] =
      (jet_counter > 0 && fabs(jet1_eta) < 5.0) ? jet1_pt : -50.;
  results[20 * idx + 9] =
      (jet_counter > 1 && fabs(jet2_eta) < 5.0) ? jet2_pt : -50.;
  results[20 * idx + 10] =
      (jet_counter > 2 && fabs(jet3_eta) < 5.0) ? jet3_pt : -50.;

  // Jet Eta (with pt cuts)
  results[20 * idx + 11] = (jet_counter > 0 && jet1_pt > 5.0) ? jet1_eta : -50.;
  results[20 * idx + 12] = (jet_counter > 1 && jet2_pt > 5.0) ? jet2_eta : -50.;
  results[20 * idx + 13] = (jet_counter > 2 && jet3_pt > 5.0) ? jet3_eta : -50.;
  ...
  results[20 * idx + 14] = n_good_jets;

  // Z-Jet1 dEta and dR, only when |jeteta1|<5 and jetpt1>5
  results[20 * idx + 15] =
      (jet_counter > 0 && fabs(jet1_eta) < 5.0 && jet1_pt > 5.0) ? z_j1_eta
                                                                 : -50.;
  results[20 * idx + 16] =
      (jet_counter > 0 && fabs(jet1_eta) < 5.0 && jet1_pt > 5.0) ? z_j1_dR
                                                                 : -50.;

  // Jet1-Jet2 dR only when both jets pass cuts
  results[20 * idx + 17] =
      (jet_counter > 1 && fabs(jet1_eta) < 5.0 && jet1_pt > 5.0 &&
  ...
  results[20 * idx + 18] =
      (jet_counter > 2 && fabs(jet1_eta) < 5.0 && jet1_pt > 5.0 &&
       fabs(jet3_eta) < 5.0 && jet3_pt > 5.0)
          ? j1_j3_dR
          : -50.;
  results[20 * idx + 19] =
      (jet_counter > 2 && fabs(jet2_eta) < 5.0 && jet2_pt > 5.0 &&
       fabs(jet3_eta) < 5.0 && jet3_pt > 5.0)
          ? j2_j3_dR
          : -50.;
