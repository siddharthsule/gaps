...
__global__ void cluster_durham(const event* events, double* results, int n) {
  ...
  results[21 * idx + 0] = counter > 0 ? log10(kt2[counter - 1 - 0]) : -50.;
  results[21 * idx + 1] = counter > 1 ? log10(kt2[counter - 1 - 1]) : -50.;
  results[21 * idx + 2] = counter > 2 ? log10(kt2[counter - 1 - 2]) : -50.;
  results[21 * idx + 3] = counter > 3 ? log10(kt2[counter - 1 - 3]) : -50.;
