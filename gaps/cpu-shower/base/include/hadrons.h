...
inline bool is_diquark(int pid) {
  int a = abs(pid);
...
inline double hadron_mass(int pid) {
  switch (abs(pid)) {
...
inline int meson_pid(int q_pid, int qbar_pid) {
...
inline int baryon_pid(int q_pid, int diq_pid) {
...
inline bool which_meson(int q_pid, int qbar_pid, int& out_pid,
                        double& out_mass) {
...
inline bool which_baryon(int pid1, int pid2, int& out_pid, double& out_mass) {
  int q_pid, diq_pid;
  if (is_diquark(pid1)) {
    diq_pid = pid1;
    q_pid = pid2;
...
inline bool which_hadron(int pid1, int pid2, int& out_pid, double& out_mass) {
