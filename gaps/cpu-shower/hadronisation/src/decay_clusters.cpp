...
// Forward declarations for hadron utility functions defined in hadrons.cpp
bool is_diquark(int pid);
bool which_hadron(int pid1, int pid2, int& out_pid, double& out_mass);
...
void hadronisation::decay_clusters(event& ev, cluster_list& cl) const {
  ...
  for (int i = 0; i < n_cl; i++) {
    ...
    if (!ok2_test || M < mass_pid1 + mass_pid2) {
      ...
      kallen(cl_mom, had_mass, 0.0, p_had, p_gamma, rho1, rho2);
    ...
    for (int attempts = 0; attempts < max_attempts; ++attempts) {
      ...
      int fl = select_qq_flavour(rho, true, rho_diq);
      ...
      double rho1 = ev.gen_random();
      double rho2 = ev.gen_random();
      kallen(cl_mom, m1_mass, m2_mass, h1, h2, rho1, rho2);
