...
const int n_light_diquarks = 9;
const int light_diquarks[n_light_diquarks] = {1103, 2101, 2103, 2203,
...
double hadron_weight(int pid1, int pid2, int had_pid) {
  /**
...
void add_candidate(int pid1, int pid2, int had_pid, candidate* out,
                          int& n) {
  /**
  ...
  if (n >= max_candidates) {
    std::cerr << "add_candidate: list full for " << pid1 << " " << pid2
              << std::endl;
    return;
  }
...
int build_candidates(int pid1, int pid2, candidate* out) {
  /**
...
candidate_table build_candidate_table() {
  /**
   * @brief build every list the cluster decay can ask for
   *
  ...
  // d, u, s, c, b on the quark side of every pair
  for (int q = 1; q <= 5; q++) {
    // Mesons: the quark against each antiquark, straight into its row
    for (int qbar = 1; qbar <= 5; qbar++) {
      t.n_meson[q - 1][qbar - 1] =
          build_candidates(q, -qbar, t.meson[q - 1][qbar - 1]);
    }
    ...
    for (int d = 0; d < n_light_diquarks; d++) {
      int n = build_candidates(q, light_diquarks[d], rows);

      if (n > 2) {
        std::cerr << "build_candidate_table: " << q << " " << light_diquarks[d]
                  << " forms " << n << " baryons" << std::endl;
        n = 2;
      }

      t.n_baryon[q - 1][d] = n;
      for (int i = 0; i < n; i++) t.baryon[q - 1][d][i] = rows[i];
    }
  }
...
// hadron_database is constant-initialised, so it is already in place when this
// runs and the lists are built against a populated table
const candidate_table candidates = build_candidate_table();

// ---------------------------------------------------------------------------
// The lightest states a cluster reaches, read off the table

candidate lightest_meson(int q, int qbar) {
  /**
...
double lightest_meson_pair_mass(int q, int qbar) {
  /**
...
bool can_produce_baryon_pair(int q, int qbar, double m_cluster) {
  /**
...
void fallback_reshuffling(event& ev, cluster_list& cl) {
  /**
  ...
  if (mass_sum > ecms) {
    std::cerr << "fallback_reshuffling: the masses the clusters decay with sum "
                 "above Ecms, the momenta are left as they are"
              << std::endl;
    return;
  }
...
double choose_decay(const double* pwt, int p1_pid, int p2_pid,
                    double m_cluster, bool want_baryon, double target,
                    int& h1_pid, double& h1_mass, int& h2_pid,
                    double& h2_mass) {
  /**
...
void hadronisation::decay_clusters(event& ev, cluster_list& cl) const {
  /**
  ...
  for (int i = 0; i < n_cl; i++) {
    ...
    bool baryonic = (rho_cat * (1.0 + pwt[3]) < pwt[3]) &&
    ...
    if (reachable) {
      ...
      double total = choose_decay(pwt, p1_pid, p2_pid, M, baryonic, -1., h1_pid,
                                  h1_mass, h2_pid, h2_mass);

      // Draw one pair with probability W / sum(W)
      double rho_pair = ev.gen_random();
      choose_decay(pwt, p1_pid, p2_pid, M, baryonic, rho_pair * total, h1_pid,
                   h1_mass, h2_pid, h2_mass);
      ...
      if (h1_pid == 0) {
        std::cerr << "decay_clusters: no pair selected for a reachable cluster"
                  << std::endl;
      }
