...
void hadronisation::kallen(vec4 p0, double m1, double m2, vec4& p1, vec4& p2,
  ...
  if (m0 < m1 + m2) {
    std::cerr << "kallen: m0 < m1 + m2!" << std::endl;
    return;
  }
  ...
  // Directions of the decay products (unit vectors by construction: sin²θ + cos²θ = 1)
