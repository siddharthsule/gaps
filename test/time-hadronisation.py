...
parser = argparse.ArgumentParser(description='Run Hadronisation Timing Analysis')
parser.add_argument('option', choices=['cpu', 'gpu', 'plot'], help='Select the option to run')
parser.add_argument('--nreps', type=int, default=10, help='Number of repetitions for each run (default: 10)')
...
time_cols = ['me_time', 'shower_time', 'hadronisation_time', 'analysis_time', 'total_time']
...
def median_and_iqr(data):
    ...
    for i in range(n_groups):
        ...
        subset_filtered = subset[(total >= lower_bound) & (total <= upper_bound)]

        print(f"Subset {i}: {len(subset_filtered)} points after outlier removal (original {nreps})")
        ...
        print(f"Subset {i}: median total={median[i][-1]:.4f}s, IQR={iqr_values[i][-1]:.4f}s")
...
if option == 'plot':
    ...
    gpu = pd.read_csv("../" + time_gpu, header=None, delimiter=',', names=time_cols)
    cpu = pd.read_csv("../" + time_cpu, header=None, delimiter=',', names=time_cols)

    gpu_med, gpu_iqr_vals = median_and_iqr(gpu)
    cpu_med, cpu_iqr_vals = median_and_iqr(cpu)
    ...
    for k, (label, color) in enumerate(zip(comp_labels, comp_colors)):
        bottom_cpu = cpu_med[:, :k].sum(axis=1) if k > 0 else np.zeros(n_groups)
        bottom_gpu = gpu_med[:, :k].sum(axis=1) if k > 0 else np.zeros(n_groups)
