...
std::string ToString(Histo2D h, std::string name) {
    ...
    ss << "# xlow\txhigh\tylow\tyhigh\tsumw\tsumw2\tsumwx\tsumwx2\tsumwy\tsumwy2\tsumwxy\tnumEntries\n";
    for (size_t i = 0; i < nBins; ++i) {
        for (size_t j = 0; j < nBins; ++j) {
            ss << std::scientific << std::setprecision(6);
            ss << h.bins[i][j].xmin << "\t" << h.bins[i][j].xmax << "\t" << h.bins[i][j].ymin << "\t" << h.bins[i][j].ymax << "\t"
                << h.bins[i][j].w << "\t" << h.bins[i][j].w2 << "\t" << h.bins[i][j].wx << "\t" << h.bins[i][j].wx2 << "\t"
                << h.bins[i][j].wy << "\t" << h.bins[i][j].wy2 << "\t" << h.bins[i][j].wxy << "\t" << static_cast<int>(h.bins[i][j].n) << "\n";
        }
    }
    ss << "END YODA_HISTO2D\n\n";
    return ss.str();
